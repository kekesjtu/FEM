...
void FEMSolver::preprocess()
{
    ...
    {
        n_ = connectivity[0].size();  // 第一个单元的节点数
    }
...
void FEMSolver::applyBoundaryConditions()
{
    ...
    // 边界条件已经在 preprocess() 阶段通过 problem_->setupBoundaryConditions() 设置好了
    ...
    for (int i : dirichlet_nodes)
    {
        ...
        for (Eigen::SparseMatrix<double>::InnerIterator it(K_global_, i); it; ++it)
        {
            ...
            if (j != i)
            {
                ...
                // 典型失败场景：Dirichlet u≠0 + 无源项（如电场拉普拉斯方程）
                                             ...
                                             int local_j)
{
    ...
    // 使用工厂创建边界单元的几何映射对象
    auto mapping = GeometryMappingFactory::createMapping(edge_coords, config_, true);
...
double FEMSolver::calculateBoundaryLoad(const Config::Boundary& boundary, int local_i)
{
    // 获取边界边的节点索引
    const auto& edge_nodes = boundary.global_node_indices_in_element;
    int boundary_nodes = config_->getBoundaryNodesPerElement();
    ...
    for (int gpIndex = 0; gpIndex < gaussPoint->getNumPoints(); ++gpIndex)
    {
        ...
        // 计算边界条件右端项的值
        // 边界条件已经在 applyBoundaryConditions 中通过 ProblemSetup 动态设置
        double q_value = boundary.bc.q_bc;
        double q_over_K = q_value / boundary.bc.K_bc;
