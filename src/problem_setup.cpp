...
void ProblemSetup::setupBoundaryConditions(std::shared_ptr<Config> config)
{
    if (!boundary_setup_func_)
    {
        throw std::runtime_error("边界条件设置函数未设置 (问题: " + name_ + ")");
    }
    boundary_setup_func_(config);
}
