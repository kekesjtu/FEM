...
void ElectrothermalSolver::solve()
{
    ...
    for (iterations_ = 1; iterations_ <= max_iter_; ++iterations_)
    {
        ...
        // ⚠️ 关键：每次求解前重新设置电场边界条件
        // 因为 electric_solver 和 thermal_solver 共享同一个 Config
        // thermal_solver->preprocess() 会覆盖边界条件
        electric_solver_->getProblem()->setupBoundaryConditions(electric_solver_->getConfig());
        ...
        // ⚠️ 关键：每次求解前重新设置热场边界条件
        thermal_solver_->getProblem()->setupBoundaryConditions(thermal_solver_->getConfig());
